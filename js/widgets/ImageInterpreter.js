...
export class ImageInterpreter extends SyncableWidgetInterpreter {
    ...
    canHandle(w, node, options) {
        ...
        // Не перехватывать output-ноды — они обрабатываются PreviewImageInterpreter
        const isOutputNode = node.type && (
            node.type.toLowerCase().includes("preview") ||
            node.type.toLowerCase().includes("save") ||
            node.type.toLowerCase().includes("show")
        );

        if (isOutputNode) return false;

        // Пропустить fake-виджеты preview (создаются grid.js для __preview__)
        if (w.name === "$$canvas-image-preview") return false;
