...
export async function applyValuePreset(savedValues, presetName = "Unknown") {
    ...
    savedValues.forEach(sv => {
        if (!sv.nodeType && !sv.nodeId) { skippedCount++; return; }
        if (!sv.widgetName) { skippedCount++; return; }

        // Собираем ВСЕ подходящие ноды (не только первую)
        let targetNodes = [];

        // По nodeId (точное совпадение)
        if (sv.nodeId) {
            const node = app.graph.getNodeById(sv.nodeId);
            if (node) targetNodes = [node];
        }

        // По типу ноды среди UI-нод
        if (targetNodes.length === 0 && sv.nodeType) {
            targetNodes = app.graph._nodes.filter(n =>
                uiNodeIds.has(n.id) &&
                n.type === sv.nodeType
            );
        }

        // Если нет UI-нод этого типа, ищем во всём графе (для глобальных пресетов)
        if (targetNodes.length === 0 && sv.nodeType) {
            targetNodes = app.graph._nodes.filter(n =>
                n.type === sv.nodeType
            );
        ...
        // Применяем к каждой подходящей ноде
        targetNodes.forEach(targetNode => {
            ...
            // Инкрементальное обновление — только если значение изменилось
